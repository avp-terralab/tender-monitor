...
test('parseCommand: /docs with valid id', () => {
  assert.deepEqual(
    parseCommand('/docs UA-2026-04-30-010542-a'),
    { cmd: 'docs', tender_id: 'UA-2026-04-30-010542-a' }
  );
});

test('parseCommand: /docs normalizes uppercase suffix', () => {
  assert.deepEqual(
    parseCommand('/docs UA-2026-04-30-010542-A'),
    { cmd: 'docs', tender_id: 'UA-2026-04-30-010542-a' }
  );
...
test('parseCommand: /docs with bot suffix', () => {
  assert.deepEqual(
    parseCommand('/docs@my_bot UA-2026-04-30-010542-a'),
    { cmd: 'docs', tender_id: 'UA-2026-04-30-010542-a' }
  );
});

test('parseCommand: /docs without args → error missing_id', () => {
  assert.deepEqual(parseCommand('/docs'), { cmd: 'docs', error: 'missing_id' });
});

test('parseCommand: /docs with garbage → error invalid_id', () => {
  assert.deepEqual(parseCommand('/docs bad-id'), { cmd: 'docs', error: 'invalid_id' });
});
...
const SAMPLE_DOCS = [
  {
    ...
    url: 'https://docs.example/td',
  },
  {
    id: 'd2', title: 'Додаток 5 (перелік).docx', documentType: 'biddingDocuments',
    datePublished: '2026-04-30T12:30:00+03:00',
    url: 'https://docs.example/d5',
  },
  {
    id: 'd3', title: 'Додаток 1 (квалкритерії).docx', documentType: 'evaluationCriteria',
    datePublished: '2026-04-30T12:15:00+03:00',
    url: 'https://docs.example/d1',
  },
];

test('formatDocs: empty docs → 📭 reply', () => {
  const reply = formatDocs({ tender_id: 'UA-X', docs: [] });
  assert.match(reply, /📭/);
  assert.match(reply, /UA-X/);
  assert.match(reply, /немає документів/);
});

test('formatDocs: groups by documentType in DOC_TYPE_ORDER', () => {
  const reply = formatDocs({ tender_id: 'UA-X', docs: SAMPLE_DOCS });
  // tenderNotice (Оголошення) appears BEFORE evaluationCriteria (Кваліф)
  // which appears BEFORE biddingDocuments (Перелік)
  const idxNotice = reply.indexOf('Оголошення');
  const idxEval = reply.indexOf('Кваліфікаційні');
  const idxBid = reply.indexOf('Перелік документів');
  assert.ok(idxNotice >= 0 && idxEval > idxNotice && idxBid > idxEval,
    `wrong order: notice=${idxNotice} eval=${idxEval} bid=${idxBid}`);
});

test('formatDocs: header shows count', () => {
  const reply = formatDocs({ tender_id: 'UA-X', docs: SAMPLE_DOCS });
  assert.match(reply, /📎 Документи .* \(3\)/);
});

test('formatDocs: each doc has title + date + download link', () => {
  const reply = formatDocs({ tender_id: 'UA-X', docs: SAMPLE_DOCS });
  assert.match(reply, /ТД\.docx/);
  assert.match(reply, /30\.04\.2026 12:00/);
  assert.match(reply, /href="https:\/\/docs\.example\/td"/);
  assert.match(reply, /завантажити/);
});

test('formatDocs: unknown documentType goes to 📝 Інше group at end', () => {
  const docs = [
    { id: 'd1', title: 'Дивний.docx', documentType: 'someUnknownType',
      datePublished: '2026-04-30T12:00:00+03:00', url: 'https://x' },
    { id: 'd2', title: 'ТД.docx', documentType: 'tenderNotice',
      datePublished: '2026-04-30T12:00:00+03:00', url: 'https://y' },
  ];
  const reply = formatDocs({ tender_id: 'UA-X', docs });
  const idxNotice = reply.indexOf('Оголошення');
  const idxOther = reply.indexOf('Інше');
  assert.ok(idxNotice >= 0 && idxOther > idxNotice);
});

test('formatDocs: within group, sorted by datePublished desc', () => {
  const docs = [
    { id: 'a', title: 'Стара версія.docx', documentType: 'tenderNotice',
      datePublished: '2026-04-29T10:00:00+03:00', url: 'https://a' },
    { id: 'b', title: 'Нова версія.docx', documentType: 'tenderNotice',
      datePublished: '2026-04-30T15:00:00+03:00', url: 'https://b' },
  ];
  const reply = formatDocs({ tender_id: 'UA-X', docs });
  const idxNew = reply.indexOf('Нова версія');
  const idxOld = reply.indexOf('Стара версія');
  assert.ok(idxNew < idxOld, `expected new before old: new=${idxNew} old=${idxOld}`);
});

test('formatDocs: HTML-escapes title and url', () => {
  const docs = [{
  ...
  const reply = formatDocs({ tender_id: 'UA-X', docs });
  assert.match(reply, /A &amp; &lt;B&gt;\.docx/);
  assert.match(reply, /https:\/\/x\?a=1&amp;b=2/);
});

test('formatDocs: missing url shows fallback', () => {
  const docs = [{
    id: 'd1', title: 'X.docx', documentType: 'tenderNotice',
    datePublished: null, url: null,
  }];
  const reply = formatDocs({ tender_id: 'UA-X', docs });
  assert.match(reply, /URL недоступний/);
});

test('formatInfo: shows 📎 Документів N when documents_count > 0', () => {
  const reply = formatInfo({
    runIso: '2026-05-08T13:00:00+03:00',
    groups: [{
      tender_id: 'UA-X',
  ...
  assert.match(reply, /📎 Документів: 7 \(\/docs UA-X\)/);
});

test('formatInfo: omits 📎 line when documents_count is 0 or absent', () => {
...
test('formatDocs: footer link to tender page', () => {
  const reply = formatDocs({ tender_id: 'UA-X', docs: SAMPLE_DOCS });
  assert.match(reply, /🌐 Тендер: https:\/\/prozorro\.gov\.ua\/tender\/UA-X/);
});
