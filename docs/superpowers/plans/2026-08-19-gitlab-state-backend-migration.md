...
#### КРОК 0: п'ять передумов — ВИКОНАНО 23.08.2026

⛔ **Не починати з Кроку 1** було правильним рішенням: п'ять речей, потрібних
для cutover, не існували, і **кожна виявилася б уже після перемикання**. Дві з
них — 0.4 і 0.5 — знайшлися лише тому, що ми перевіряли передумови по одній, а
не «загалом».

- [x] **0.1. Право писати стан у `main`. ЗНЯТО — стан переїхав із `main`.**

  Заміряно, не припущено: пуш токеном проєкту в `main` віддав
  `You are not allowed to push code to protected branches`. Гілка захищена
  (push і merge — Maintainers 40), а токен `project_14_bot_…` має Developer (30).

  Спершу планували підняти токен до Maintainer. **Відмовились** після питання
  власника «що і хто робитиме цим токеном»: з усього, що робить цей токен —
  читання й запис файлів стану плюс історія комітів, — Maintainer потрібен
  рівно для ОДНОЇ здатності. А додатково дав би читання й запис CI-змінних
  проєкту (тобто `CLOUDFLARE_API_TOKEN`), керування захистом гілок і раннерами.
  Постійний набір повноважень, з якого використовується один пункт.

  **Рішення: прод-стан лежить в окремій незахищеній гілці `prod-state`.** Токен
  лишається Developer, `main` лишається захищеною. Головний аргумент — форма:
  staging уже три дні працював саме так (незахищена гілка стану + Developer), і
  це та конфігурація, яку ми перевірили наскрізь. Варіант із Maintainer зробив
  би прод ІНШИМ за формою, ніж перевірене.

  Бонус: стан перестає змішуватися з кодом — саме через це змішування ми весь
  час натикались на «GitHub забіг уперед» і мерджили.

  Ціна, заплачена свідомо: `fetchLatestDeployCommit` тепер читає окрему **гілку
  коду** (`GITLAB_CODE_REF`), бо шукає останній НЕ службовий коміт — а в гілці
  стану всі коміти службові, і стрічка «останній деплой» у /status спорожніла б
  за добу. Це не латка: функція про деплой коду нарешті читає код.
  `fetchAuditLog` навпаки лишився на гілці стану — `audit:`-коміти це і є стан.

  Гілку `prod-state` створено з `main`, unprotected.

- [x] **0.2. Джоба `monitor-production` — створена.**

  Її не існувало взагалі, тобто «увімкнути Scheduled Pipeline» у Кроці 3
  нічого б не запустило. Та сама пастка вже спрацювала зі staging: розклад
  стояв активним і мовчав, бо на гілці не було `.gitlab-ci.yml`.

  Спільний блок «закомітити стан і запушити» винесено в шаблон
  `.monitor-commit-push`, гілку задає `STATE_BRANCH` — 78 рядків шелу не
  дублювали свідомо: у цьому проєкті дубльовані шляхи розходяться.

  ⚠️ Пише в `prod-state`. Запускається лише розкладом із
  `SCHEDULE_TARGET=production`; сам розклад створюється НЕАКТИВНИМ, інакше писав
  би реальним користувачам паралельно з погодинним `monitor.yml` на GitHub.
  Увімкнути — рівно на Кроці 3.

  ⚠️ Дорогою я зламав це власною правкою: вирізав старий текст по рядку
  `exit 1`, вважаючи його кінцем джоби, а після скрипта йшов ще `rules:` — він
  приліпився до нової джоби другим ключем, і в YAML переміг останній. Прод-монітор
  дістав умову staging і побіг на тестовому розкладі. `POST /ci/lint` при цьому
  відповів `valid: true` — на дубльований ключ він не гейт. Спіймав живий цикл.
  Тепер є `test/gitlab-ci.test.mjs`: немає дубльованих ключів, кожен монітор
  слухає свій розклад і пише у свою гілку, і в блоці немає умови ЧУЖОГО розкладу.

- [x] **0.3. GitLab-параметри в `[env.production]` — додані.**

  Було лише `STATE_BACKEND`. Тепер `GITLAB_PROJECT_ID`, `GITLAB_REF`
  (`prod-state`) і `GITLAB_CODE_REF` (`main`) — щоб cutover був зміною ОДНОГО
  значення, а не двома правками, з яких другу легко забути.

  Гейт `worker/test/wrangler.test.mjs`: обидва оточення мусять мати ці
  параметри незалежно від прапорця, власний KV-байндинг, і гілка стану не
  збігається з гілкою коду.

- [x] **0.4. Секрет `GITLAB_TOKEN` у продовому Worker'і — доданий.**

  Знайдено переліком секретів: у staging він був, у **production — ні**, там
  лежав лише `GITHUB_PAT`. Без нього після перемикання прапорця `gitlab.mjs`
  отримав би `undefined` і кожен виклик віддав би 401. Той самий клас, що 0.3.
  Заведено оператором (секрет), підтверджено через API.

- [x] **0.5. Деплой не застосовував `[env.production]` ВЗАГАЛІ. Найпідліша.**

  `worker-deploy.yml` жене `npx wrangler deploy` **без `--env`**, а топ-рівень
  `wrangler.toml` має ТЕ САМЕ ім'я скрипта (`tender-monitor-bot`). Тобто живий
  прод працював із топ-рівневим конфігом, і цілий блок `[env.production]`
  виглядав живим, не бувши таким.

  Наслідок на cutover: прапорець перемкнули б у блоці, якого ніхто не читає,
  побачили б, що поведінка не змінилась, і шукали б причину де завгодно.
  Разом із ним мертвими були б і всі параметри з 0.3.

  Виправлено на `--env production`; ім'я скрипта в обох шляхах однакове, тож
  новий Worker не створюється й секрети зберігаються. Перевірено живим деплоєм:
  `/status` на проді показав `Деплой: 8a8df08`, `GitHub auth: OK`, 11 тендерів.
  `deploy-production` у `.gitlab-ci.yml` `--env production` уже вживав —
  розбіжність була саме в GitHub-шляху, і саме він жене прод.

  Гейт: кожна команда `wrangler deploy` у воркфлоу мусить мати `--env production`.

**Лишилось перед Кроком 1** (обидва пункти — за оператором, це секрети й жива
конфігурація):

1. Дві CI-змінні справжнього бота — `TELEGRAM_BOT_TOKEN` і
   `TELEGRAM_CHAT_ID` (`ci.mjs` читає рівно ці імена). Обидві **masked** і
   **unprotected**: прод-розклад іде на `prod-state`, а вона НЕ захищена, тож
   protected-змінна до джоби просто не доїде — і виглядатиме це як «токена
   немає». Та сама пастка, що вже спрацювала зі staging.
2. Створити прод-розклад — `ref: prod-state`, `cron: '0 * * * *'`,
   `SCHEDULE_TARGET=production` — і лишити його **НЕАКТИВНИМ**. Вмикається аж
   на Кроці 3, одночасно з вимкненням GitHub-розкладу.

Потрібні вони прод-моніторові, тобто аж на Кроці 3 — але заводити краще
заздалегідь, щоб cutover був перемиканням, а не пригадуванням.

⚠️ **Крок 1 змінюється:** стан переноситься не в `main`, а в **`prod-state`**.
...
- [ ] **Крок 1: Одноразовий перенос реального стану — у `prod-state`, НЕ в `main`**

```bash
cd "C:/Users/andre/Desktop/AI/tender-monitor"
git fetch origin main            # GitHub — джерело живого стану
git fetch gitlab prod-state
git checkout -B cutover-state gitlab/prod-state
git checkout origin/main -- watchlist.json watched_entities.json _state/
git add watchlist.json watched_entities.json _state/
git commit -m "cutover: перенести живий стан прода з GitHub"
git push gitlab HEAD:prod-state
```

⚠️ **Це природна точка зупинки.** `prod-state` ніхто не читає, доки не зроблено
Крок 2 — прод і далі працює через GitHub. Можна перенести, звірити кількість
тендерів і замовників, і спокійно спинитись.

Звірка після переносу (а не «на око»): порівняти `watchlist.json` у
`origin/main` і `gitlab/prod-state` — має бути побайтово однаково.

- [ ] **Крок 2: Перемкнути прод-прапорець**

У `wrangler.toml` в `[env.production]` змінити `STATE_BACKEND = "github"` на
`"gitlab"`. Решта вже на місці з Кроку 0.3 — `GITLAB_PROJECT_ID = "14"`,
`GITLAB_REF = "prod-state"`, `GITLAB_CODE_REF = "main"`. **Це має бути зміна
одного значення; якщо доводиться правити щось іще — щось пропустили в Кроці 0.**

Деплой: пуш у GitHub `main` (воркфлоу вже жене `--env production`, див. 0.5),
або вручну `cd worker && npx wrangler deploy --env production`.

- [ ] **Крок 3: Перемкнути cron**

Вимкнути schedule в GitHub Actions (`monitor.yml`) через GitHub UI/API. **Одночасно** увімкнути production Scheduled Pipeline у GitLab: `ref: prod-state`, `cron: '0 * * * *'`, змінна `SCHEDULE_TARGET=production`.

⚠️ `ref: prod-state`, а не `main` — стан живе там (див. Крок 0.1).

⚠️ Розклад до цього моменту вже створений, але **неактивний**: активним він писав би реальним користувачам паралельно з `monitor.yml`. Тут його лише вмикають.

⚠️ «Одночасно» — не побажання: поки обидва розклади активні, сповіщення дублюються; поки обидва вимкнені, нові тендери не знаходяться взагалі.
